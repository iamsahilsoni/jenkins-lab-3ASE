...
public class LibraryforTrigonometry {
	public static final double PI = 3.141592; //Value of π
	...
	*/	
	...
	public static double sin(double angle) throws Exception {
		double sin=0;
		double normalizedAngle = normalizeRadian(angle); //  x between [-2*PI, 2*PI]
		for(int i=0;i<=10;i++) {
			sin = sin + (power(-1, i)* power(normalizedAngle,2*i+1 )/fact(2*i+1));
		}
		return sin;
	}
	...
	public static double cos(double angle) throws Exception {
		...
		double normalizedAngle = normalizeRadian(angle);// x between [-2*PI, 2*PI]	
		for(int i=0;i<=7;i++) {	
			cos = cos + (power(-1, i)*power(normalizedAngle, 2*i)/fact(2*i));
		}
		return cos;
	}
